...
function App() {
  ...
  // select 동작을 하는 프로그렘을 작성할때
  // select객체안에 있는 값은 전부 select동작을 하기위해 만든 것이라고 알수 있음
  ...
  // 유저가 선택한 값은 주시하자 선택하면 데이터 바뀌상태로 다시 그려줘야 하기 때문에
  // 유저의 선택 useState
  ...
  function userClick(_select){
    ...
    if(player.name === compter.name){
      ...
      let result = compter.name === "보" ? "이겼음" : "졌다"
      setResult(result);
    }else if(player.name === "바위"){
      let result = compter.name === "가위" ? "이겼음" : "졌다"
      setResult(result);
    }else if(player.name === "보"){
      let result = compter.name === "바위" ? "이겼음" : "졌다"
      setResult(result);
    }
  }
