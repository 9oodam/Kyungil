// TCP server, Client 둘 다 만들어보기

// 1) 내장 모듈 net 가져오기, PORT 생성
const net = require("net");
const PORT = 6000;

// 2) body 만들기
const body = Buffer.from("<div><p>Hello nodejs!</p></div>")

// 클라이언트와 서버가 요청 응답으로 주고 받는 메시지는 헤더와 바디로 나눠지고
// 헤더의 내용은 전달하는 메시지의 정보를 전달하고
// 바디에는 전달하는 데이터의 내용이 들어있음

// 3) response
// header, body로 구분해서 읽음
// response header + body = request message
const response = `HTTP/1.1 200 OK
Connection : keep-alive
Keep-Alive : timeout=4
Content-type : text/html
Content-length : ${body.length}
${body.toString()}
`
// HTTP1.1 기본 버전 프로토콜
// GET / URL / HTTP1.1
// host : 127.0.0.1:8080
// ....
// Content-type : text/html

// 요청 메소드 // 
// GET : 데이터의 요청 (필요한 데이터를 응답 받는 것(가져오는 것))
// POST : 데이터의 입력 (데이터를 추가)
// PUT : 데이터의 수정
// DELETE : 데이터를 삭제
// OPTIONS : 웹 서버가 지원하는 메소드의 종류를 요청

// HTTP 프로토콜 버전
// HTTP 버전은 1.0 1.1. 2.0이 있는데 우리는 1.1 배울 예정

// 상태 코드
// 요청에 대한 응답의 결과를 나타낸 숫자 코드
// 1xx : 거의 없음
// 2xx : 성공
// 3xx : 리다이렉트
// 4xx : 페이지가 없음 (404 error)
// 5xx : 서버 터짐

// 가장 많이 사용하는 상태코드는 200과 404 성공과 실패
// Connection : 클라이언트와 서버의 연결 상태 keep-alive 속성은 클라이언트가 다음 요청을 보낼때까지 연결 유지
// Keep-Alive : 연결을 유지하는 시간을 지정 timeout=4 -> 연결을 4초동안 유지하고 종료
// Conetent-type : 전송되는 데이터의 타입 text/html => HTML 타입의 데이터 전송
// Content-length : 전송되는 데이터의 길이 데이터의 끝을 알려주는 역할

// 서버 객체 생성
const server = net.createServer((client) => {
    // 클라이언트가 접속시 실행 
    // 인코딩 설정
    // setEncoding 메소드로 인코딩 방식 설정
    client.setEncoding('utf8');

    // 클라이언트가 데이터를 보내서 데이터를 받으면 실행되는 콜백 함수
    client.on("data", (data) => {
        console.log(data); // 데이터의 타입 buffer
        // write 메소드로 클라이언트에 응답 보냄
        client.write(response);
    })
    // 클라이언트와 연결이 종료되면 실행되는 이벤트
    client.on('close', () => {
        console.log("server off :(");
    })
})

server.on("connection", () => {
    console.log("client connected :)")
})

server.listen(PORT, () => {
    console.log("server opened well, PORT: " + PORT);
})