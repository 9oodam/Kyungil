...
contract Baseball {
    ...
    // 게임의 횟수
    // constant : 변경되지 않는 상태변수 (컴파일 될 때 한 번 배정되고 런타임 때 수정되지 않음)
    uint256 private constant GAME_COUNT = 5;

    // 게임당 지불해야하는 이더
    uint256 private ticket = 5 ether;

    // 정답의 값을 담아놓을 변수 (컴퓨터가 정할 랜덤 값, 3자리수 숫자)
    ...
    // 컨트랙트 생성자 (컨트랙트가 배포가 되면 딱 한 번 실행)
    constructor () { 
        // 최초에 딱 한번 배포자가 상태변수에 담기고
        owner = msg.sender;

        // keccak256(x) : 솔리디티에서 랜덤값을 뽑을 때 사용, 매개변수를 해시값으로 변경
